...
class ComplexitySnippetsTest extends FunSuite {
  ...
  test("Linear search the elem 7 in array [6, 1, 3, 0, 5, 6] should return index -1") {
    val elements:Array[Int] = Array(6, 1, 3, 0, 5, 6)
    val indexElem = ComplexitySnippets.search(elements, 7)
    assert(indexElem === -1)
  }
  ...
  test("Multiplying identity matrix by a vector should return the same vector"){
    ...
    val resultVector = ComplexitySnippets.multiply(matrix, vector)
  ...
  test("Binary search the elem 12 in array [3, 4, 6, 8, 12, 14] should return index 4"){
    ...
    val indexElem = ComplexitySnippets.binarySearch(elements, 12)
    assert(indexElem === 4)
  }

  test("Binary search the elem 1 in array [3, 4, 6, 8, 12, 14] should return index -1"){
    val elements:Array[Int] = Array(3, 4, 6, 8, 12, 14)
    val indexElem = ComplexitySnippets.binarySearch(elements, 1)
    assert(indexElem === -1)
  }

  test("Binary search the elem 8 in array [3, 4, 6, 8, 12, 14] should return index 3"){
    val elements:Array[Int] = Array(3, 4, 6, 8, 12, 14)
    val indexElem = ComplexitySnippets.binarySearch(elements, 8)
    assert(indexElem === 3)
  }
  ...
  test("Sort array [4, 3, 6, 14, 12, 8] should return array [3, 4, 6, 8, 12, 14]"){
    ...
    val result = ComplexitySnippets.mergeSort(elements)
  ...
  test("Merge Array [1, 2, 3, 4] and Array [5, 6] should return Array [1,2,3,4,5,6]"){
    ...
    val result = ComplexitySnippets.merge(part1, part2)
    assert(result.deep == expectedResult.deep)
  }

  test("Min on array [4, 6, 1, 0, 4, 2] should return 0"){
    val inputArray = Array(4, 6, 1, 0, 4, 2)
    val expectedReturn  = 0
    val actualResult = ComplexitySnippets.min(inputArray)
    assert(expectedReturn == actualResult)
  }

  test("Min on empty array [] should return Int.maxInt"){
    val inputArray: Array[Int] = Array()
    val expectedReturn  = Int.MaxValue
    val actualResult = ComplexitySnippets.min(inputArray)
    assert(expectedReturn == actualResult)
  }
